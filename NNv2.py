...
def _forward_propagation(x: np.ndarray, h1: np.ndarray, h2: np.ndarray, o: np.ndarray, weightIH1: np.ndarray, 
    ...
    h2 += np.dot(weightH1H2, h1) + biasH2
    h2 *= (h2 > 0)
    
    o += softmax(np.dot(weightH2O, h2) + biasO)
...
def _backward_propagation(inp: np.ndarray, h1: np.ndarray, h2: np.ndarray, o: np.ndarray, exp: np.ndarray, weightIH1: np.ndarray,
    ...
    dB2 = np.dot(weightH2O.transpose(), dBO) * (h2 > 0)
    dWH1H2 = np.dot(dB2, h1.transpose())
    
    dB1 = np.dot(weightH1H2.transpose(), dB2) * (h1 > 0)
...
class NeuralNetwork:
    def __init__(self, ep: int, batch: int, lr: float, toLoad: bool, num_h1: int, num_h2: int, num_o: int) -> None:
        ...
        if toLoad:
            ...
            self.biasH2 = np.random.random((num_h2,1)).astype(np.float64) / 10
    ...
    def overfit(self) -> float:
        ...
        for i in range(self.vector_images.shape[0]):
            _forward_propagation(self.vector_images[i], self.h1, self.h2, self.o, self.weightIH1, self.weightH1H2, self.weightH2O, self.biasH1, self.biasH2, self.biasO)
            ans = self.get_ans()
